...
Box::Box(glm::vec2 position, glm::vec2 velocity, float mass, float width, float height, float orientaion, float angularVelocity, glm::vec4 colour) : RigidBody(ShapeType::BOX, position, velocity, orientaion, angularVelocity, mass)
{
	m_extents.x = width / 2;
	m_extents.y = height / 2;
	m_colour = colour;

	float cs = cosf(orientaion);
	float sn = sinf(orientaion);
...
void Box::fixedUpdate(glm::vec2 gravity, float timeStep)
{
	...
	float cs = cosf(getOrientation());
	float sn = sinf(getOrientation());
	m_localX = glm::normalize(glm::vec2(cs, sn));
	m_localY = glm::normalize(glm::vec2(-sn, cs));
}
...
void Box::draw()
{
	...
	//Debug
	aie::Gizmos::add2DLine(m_position, m_position + m_extents.x * m_localX, { 1,0,0,1 });
	aie::Gizmos::add2DLine(m_position, m_position + m_extents.y * m_localY, { 0,0,1,1 });
}
